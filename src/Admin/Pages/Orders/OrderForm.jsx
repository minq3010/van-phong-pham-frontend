...
const createInitialFormState = (customerType = "retail") => ({
  customerName: "",
  phone: "",
  address: "",
  email: "",
...
const OrderForm = ({
  ...
  const [formData, setFormData] = useState(() =>
    createInitialFormState(defaultCustomerType)
  );
  ...
  }, [resetSignal, defaultCustomerType]);

  useEffect(() => {
    if (forceCustomerType && formData.customerType !== forceCustomerType) {
      setFormData((current) => ({
        ...current,
        customerType: forceCustomerType,
      }));
    }
  ...
  useEffect(() => {
    ...
    setFormData((current) => ({
      ...current,
      customerName:
        prefillCustomer.customerName ?? current.customerName ?? "",
      phone: prefillCustomer.phone ?? current.phone ?? "",
      email: prefillCustomer.email ?? current.email ?? "",
      address: prefillCustomer.address ?? current.address ?? "",
    }));
  }, [prefillCustomer]);
  ...
  const availableVouchers = useMemo(
    () => vouchers.filter((item) => item.isActive === true),
    [vouchers]
  );
  ...
  const getLinePricing = (item) => {
    ...
    const isWholesaleCustomer = effectiveCustomerType === "wholesale";
  ...
  const handleInvoiceToggle = (isEnabled) => {
    updateFormField("invoiceRequested", isEnabled);
    setIsInvoiceModalOpen(isEnabled);
  };

  const productPreview = formData.products
  ...
  const validateForm = (dataToValidate = formData) => {
    ...
    if (dataToValidate.invoiceRequested) {
      ...
      const trimmedInvoiceAddress =
        dataToValidate.invoiceInfo.invoiceAddress.trim();

      if (!trimmedCompanyName) {
        nextErrors.invoiceInfo.companyName =
          "Vui lòng nhập tên công ty / đơn vị.";
      }
      ...
      if (!trimmedInvoiceEmail) {
        nextErrors.invoiceInfo.invoiceEmail =
          "Vui lòng nhập email nhận hóa đơn.";
      } else if (!EMAIL_REGEX.test(trimmedInvoiceEmail)) {
        nextErrors.invoiceInfo.invoiceEmail =
          "Email nhận hóa đơn không đúng định dạng.";
      }

      if (!trimmedInvoiceAddress) {
        nextErrors.invoiceInfo.invoiceAddress =
          "Vui lòng nhập địa chỉ xuất hóa đơn.";
      } else if (trimmedInvoiceAddress.length < 8) {
        nextErrors.invoiceInfo.invoiceAddress =
          "Địa chỉ xuất hóa đơn cần chi tiết hơn.";
      }
    }
  ...
  if (isLoading) {
    return <Spin className="w-full flex items-center justify-center py-5" />;
  }

  if (availableProducts.length === 0) {
    return <Empty description="Thiếu dữ liệu sản phẩm để tạo đơn" />;
  }
    ...
    >
      <div className="admin-order-modal-hero">
        <div className="admin-order-modal-hero-card">
          ...
          <strong>
            {effectiveCustomerType === "wholesale" ? "Khách sỉ" : "Khách lẻ"}
          </strong>
        </div>
        <div className="admin-order-modal-hero-card">
          ...
          <div className="border rounded-3 p-3 h-100 admin-order-products-card">
            <div className="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
              <div>
                ...
                <p className="text-muted mb-0">
                  Đã chọn {validProducts.length} dòng - {totalQuantity} sản phẩm.
                </p>
              </div>
              <button
                type="button"
            ...
            {productPreview.length > 0 && (
              <div className="mt-3">
                <div className="text-muted mb-2">Xem nhanh sản phẩm đã chọn</div>
                  ...
                  {productPreview.map((item) => (
                    ...
                    >
                      <div>
                        ...
                        <div className="text-muted">Màu: {item.color}</div>
                      </div>
